...
#input a user's steamid as a string and the full data set. The function look up his data and drop it 
...
def recommendation(users_steam_id, total_data1):
    ...
    nn = neighbors.KNeighborsClassifier(n_neighbors=3, metric='euclidean')
    nn.fit(total_data,labels)
    users_data = users_data.drop(['Steam_id'], axis=1)
    users_data = users_data.values[0]
    similar_users0 = nn.kneighbors([users_data])
    similar_users0 = similar_users0[1]
    all_three_users = []

    for i in range(3):
        user_index = similar_users0[0][i]
        sim_users_ids = labels[user_index]
        all_three_users.append(sim_users_ids)

    final_suggestion = []

    for i in range(3):
        similar_user = str(all_three_users[i])
